...
public class RedisConfig {
    ...
    public static JedisPoolConfig getJedisPoolConfig(){
        ...
        config.setMaxIdle(Integer.parseInt(maxIdle.trim()));// 链接池中最大空闲的连接数,默认为8.
        ...
        config.setSoftMinEvictableIdleTimeMillis(-1);//连接空闲的最小时间，达到此值后空闲链接将会被移除，且保留“minIdle”个空闲连接数。默认为-1.  
        config.setNumTestsPerEvictionRun(3);  //对于“空闲链接”检测线程而言，每次检测的链接资源的个数。默认为3.
        config.setTestOnBorrow(false); //向调用者输出“链接”资源时，是否检测是有有效，如果无效则从连接池中移除，并尝试获取继续获取。默认为false。建议保持默认值. 
        config.setTestOnReturn(false);  //向连接池“归还”链接时，是否检测“链接”对象的有效性。默认为false。建议保持默认值
        config.setTestWhileIdle(false);//向调用者输出“链接”对象时，是否检测它的空闲超时；默认为false。如果“链接”空闲超时，将会被移除。建议保持默认值.
    ...
    public static JedisPool getJedisPool(){
        ...
        String port=PropertiesUtil.getValue("redis.pool.port");
        String expire=PropertiesUtil.getValue("redis.expire");
        JedisPool pool = new JedisPool(config, host.trim(), Integer.parseInt(port.trim()), Integer.parseInt(expire.trim()));  
        return pool;
    }
